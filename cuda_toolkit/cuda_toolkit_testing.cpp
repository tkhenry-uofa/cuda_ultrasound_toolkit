...
bool readi_beamform_raw(const int16_t* input, PipelineParams params, cuComplex** volume)
{
	...
	hadamard::readi_staggered_decode(Session.d_converted, Session.d_decoded, Session.d_hadamard, 128, 1);
...
bool readi_beamform_double(const int16_t* input, PipelineParams params, cuDoubleComplex** volume)
{
	uint3 dec_data_dims = *(uint3*)&(params.decoded_dims);
	uint2 raw_data_dims = *(uint2*)&(params.raw_dims);
	size_t total_count = dec_data_dims.x * dec_data_dims.y * dec_data_dims.z;
	...
	double *d_converted, *d_decoded;
	cuDoubleComplex *d_complex;

	CUDA_RETURN_IF_ERROR(cudaMalloc((void**)&(d_converted), total_count * sizeof(double)));
	CUDA_RETURN_IF_ERROR(cudaMalloc((void**)&(d_decoded), total_count * sizeof(double)));
	CUDA_RETURN_IF_ERROR(cudaMalloc((void**)&(d_complex), total_count * sizeof(cuDoubleComplex)));

	hilbert::plan_double_hilbert(Session.decoded_dims.x, Session.decoded_dims.y * params.readi_group_size);

	i16_to_f::convert_data_double(d_input, d_converted);

	hadamard::double_decode(d_converted, d_decoded);

	hilbert::double_hilbert_transform(d_decoded, d_complex);

	//CUDA_RETURN_IF_ERROR(cudaMemset(d_complex, 0x00, total_count * sizeof(cuDoubleComplex)));
	//CUDA_RETURN_IF_ERROR(cudaMemcpy2D(d_complex, 2 * sizeof(double), d_decoded, sizeof(double), sizeof(double), total_count, cudaMemcpyDefault));


	cuDoubleComplex* d_volume;

	CUDA_RETURN_IF_ERROR(cudaMalloc(&(d_volume), vol_config.total_voxels * sizeof(cuDoubleComplex)));

	float samples_per_meter = params.array_params.sample_freq / params.array_params.c;
	*volume = (cuDoubleComplex*)malloc(vol_config.total_voxels * sizeof(cuDoubleComplex));

	std::cout << "Starting beamform" << std::endl;
	std::cout << "First value: " << sample_value_i16(d_input) << std::endl;
	std::cout << "First converted value: " << sample_value_d(d_converted) << std::endl;
	std::cout << "First decoded value: " << sample_value_d(d_decoded) << std::endl;
	std::cout << "First complex value: " << format_d_cplx(sample_value_d_cplx(d_complex)) << std::endl;
	std::cout << "Second complex value: " << format_d_cplx(sample_value_d_cplx(d_complex +1)) << std::endl;

	beamformer::double_beamform(d_volume, d_complex, *(float3*)params.focus, samples_per_meter);

	CUDA_RETURN_IF_ERROR(cudaDeviceSynchronize());
	CUDA_RETURN_IF_ERROR(cudaMemcpy(*volume, d_volume, vol_config.total_voxels * sizeof(cuDoubleComplex), cudaMemcpyDefault));
