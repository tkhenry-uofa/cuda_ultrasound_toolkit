...
namespace data_conversion
{
    ...
    bool
    DataConverter::convert_i16(const int16_t* d_input, float2* d_output, uint2 input_dims, uint3 output_dims)
    {
        if (!_d_channel_mapping)
        {
        ...
        kernels::convert_to_cf32<int16_t><<<grid_dim, block_dim >>>(d_input, d_output, input_dims, output_dims);
        CUDA_RETURN_IF_ERROR(cudaGetLastError());
    }

    bool
    DataConverter::convert_f32(const float* d_input, float2* d_output, uint2 input_dims, uint3 output_dims)
    {
        if (!_d_channel_mapping)
        {
            return false; // Channel mapping not set
        ...
        kernels::convert_to_cf32<float><<<grid_dim, block_dim >>>(d_input, d_output, input_dims, output_dims);
        CUDA_RETURN_IF_ERROR(cudaGetLastError());
    }
}