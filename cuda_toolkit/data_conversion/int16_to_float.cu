...
*/
__global__ void
i16_to_f::_kernels::short_convert(const i16* input, float* output, uint2 input_dims, defs::RfDataDims output_dims, bool rx_rows)
{
...
i16_to_f::convert_data(const i16* d_input, float* d_output, uint2 input_dims, defs::RfDataDims output_dims, bool rx_rows)
{
	...
	_kernels::short_convert << <grid_dim, block_dim >> > (d_input, d_output, input_dims, output_dims, rx_rows);
