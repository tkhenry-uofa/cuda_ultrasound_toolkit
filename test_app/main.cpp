...
bool beamform_double()
{
	...
	for (int g = 0; g < max_beamforms; g++)
	{
		...
		cuDoubleComplex* volume = nullptr;
		size_t output_size = full_bp->raw.output_points.x * full_bp->raw.output_points.y * full_bp->raw.output_points.z * sizeof(cuDoubleComplex);

		std::cout << "Starting pipeline " << g + 1 << std::endl;
		readi_beamform_double(data_buffer, params, &volume);
