...
__host__
bool cuda_fft(const std::vector<float>& re_input, std::vector<std::complex<float>>** im_output, defs::RfDataDims dims)
{
	...
	*im_output = new std::vector<std::complex<float>>(data_size);
	...
	cuda_result = cudaMemcpy(d_input, re_input.data(), sizeof(float) * data_size, cudaMemcpyHostToDevice);
	...
	// Copy out results
	cuda_result = cudaMemcpy((*im_output)->data(), d_output, sizeof(std::complex<float>) * data_size, cudaMemcpyDeviceToHost);
